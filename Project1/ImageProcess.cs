...
namespace ImageProcess
{
    public class ImageProcess :ImageEditor
    {
        ...
        {
            //I coppied the OnFilterLowpass function you provided for the framework of this function
            ...
            for (int r = 0; r < image.PostImage.Height; r++)
            {
                for (int c = 0; c < image.PostImage.Width; c++)
                {
                    ...
                    pixel = Color.FromArgb(FindMedianPixel(rPixels), FindMedianPixel(gPixels), FindMedianPixel(bPixels));
                    image.PostImage.SetPixel(c, r, pixel);
                }
            }
        }
        ...
        private static int FindMedianPixel(List<int> pixels)
        {
